...
function normalizeRegistryKits(registry) {
...
async function listKitDirs() {
  const tree = await httpGetJson(
    `${GITHUB_API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/kits`
  );
  return tree
...
async function getKitFileTree(kitName) {
  return httpGetJson(
    `${GITHUB_API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/kits/${kitName}`
  );
}

async function getKitFile(kitName, relativePath) {
  return httpGetText(`${RAW_BASE}/kits/${kitName}/${relativePath}`);
}

async function listKitFlows(kitName) {
  let entries;
  try {
    entries = await httpGetJson(
      `${GITHUB_API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/kits/${kitName}/flows`
    );
  } catch {
    return [];
  }
